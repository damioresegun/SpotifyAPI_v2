...
def ensure_playlist(sp, user_id: str, name: str, public: bool = False) -> str:
    """Return playlist ID, creating it if necessary."""
    limit, offset = 50, 0
    while True:
...
def get_playlist_tracks(sp, playlist_id: str) -> list[str]:
    """Return list of track URIs contained in the playlist."""
    uris: list[str] = []
    limit, offset = 100, 0
    while True:
        ...
        for it in res.get("items", []):
            t = it.get("track")
            if t and t.get("uri"):  # FIX: uri not url
                uris.append(t["uri"])
        if res.get("next"):
            offset += limit
        else:
...
def add_tracks_dedup(sp, playlist_id: str, new_uris: Iterable[str]) -> int:
    """Add only new tracks (by URI) to a playlist; return count actually added."""
